...
template<int order>
inline float stencil(float *prev_grid, int gx, int x, int y, float xcfl, float ycfl) {
  if (order == 2) {
    return prev_grid[x + gx * y] + 
    xcfl * (prev_grid[x+1 + gx * y] + prev_grid[x-1 + gx * y] - 2 * prev_grid[x + gx *  y]) + 
    ycfl * (prev_grid[x + gx *(y+1)] + prev_grid[x + gx *(y-1)] - 2 * prev_grid[x + gx * y]);
  }
  else if (order == 4) {
    return prev_grid[x + gx * y] + 
    xcfl * (   -prev_grid[x+2 + gx * y] + 16 * prev_grid[x+1 + gx * y] -
     30 * prev_grid[x + gx * y] + 16 * prev_grid[x-1 + gx * y] - prev_grid[x-2 + gx *  y]) + 
    ycfl * (   -prev_grid[x +  gx * (y+2)] + 16 * prev_grid[x + gx * (y+1)] -
     30 * prev_grid[x + gx * y] + 16 * prev_grid[x + gx * (y-1)] - prev_grid[x + gx * (y-2)]);
  }
  else if (order == 8) {
    return prev_grid[x + gx * y] + 
    xcfl * (   -9*prev_grid[x+4 + gx * y] + 128 * prev_grid[x+3 + gx *  y] - 1008 * prev_grid[x+2 + gx * y] + 8064 * prev_grid[x+1 + gx *  y] -
     14350 * prev_grid[x + gx *  y] +
     8064 * prev_grid[x-1 + gx * y] - 1008 * prev_grid[x-2 + gx * y] + 128 * prev_grid[x-3 + gx *  y] -9 * prev_grid[x-4 + gx * y]) +
    ycfl * (   -9*prev_grid[x + gx * (y+4)] + 128 * prev_grid[x + gx * (y+3)] - 1008 * prev_grid[x + gx * (y+2)] + 8064 * prev_grid[x + gx * (y+1)] -
     14350 * prev_grid[x + gx * y] + 
     8064 * prev_grid[x + gx * (y-1)] - 1008 * prev_grid[x + gx * (y-2)] + 128 * prev_grid[x + gx * (y-3)] - 9 * prev_grid[x + gx * (y-4)]);
  }
  else {
...
double cpuComputation(Grid &curr_grid, const simParams &params) {
  ...
  for (int i = 0; i < params.iters(); ++i) {
    ...
    updateBCsOnly(curr_grid, prev_grid, params);
    
    // apply stencil
    if (params.order() == 2) {
      for (int y = borderSize; y < ny + borderSize; ++y) {
        for (int x = borderSize; x < nx + borderSize; ++x) {
          curr_grid.hGrid_[x + gx *  y] = stencil<2>(prev_grid.hGrid_.data(), gx, x, y, xcfl, ycfl);
        }
      }
    }
      ...
      for (int y = borderSize; y < ny + borderSize; ++y) {
        for (int x = borderSize; x < nx + borderSize; ++x) {
          curr_grid.hGrid_[x + gx *  y] = stencil<4>(prev_grid.hGrid_.data(), gx, x, y, xcfl, ycfl);
        }
      }
    }
      ...
      for (int y = borderSize; y < ny + borderSize; ++y) {
        for (int x = borderSize; x < nx + borderSize; ++x) {
          curr_grid.hGrid_[x + gx *  y] = stencil<8>(prev_grid.hGrid_.data(), gx, x, y, xcfl, ycfl);
        }
      }
    }
  }
...
int checkErrors(const Grid &ref_grid, const Grid &gpu_grid, const simParams &params, std::string filename, std::vector<double>& errors)
{
  ...
  for (int x = 0; x < params.gx(); ++x) {
    for (int y = 0; y < params.gy(); ++y) {
      if (!AlmostEqual2sComplement(ref_grid.hGrid_[y * params.gx() + x], gpu_grid.hGrid_[x + params.gx() * y], 512)) {
...
int main(int argc, char *argv[])
{
  ...
  Grid grid(params.gx(), params.gy(), params.ic());
  setBCs(grid, params);

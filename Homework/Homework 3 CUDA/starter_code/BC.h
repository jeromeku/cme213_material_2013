...
__global__
void gpuUpdateBCsOnly(float *curr, float *prev, int gx, int gy, int b, float scaling_factor) {
...
public:
  ...
  void updateBC(float * curr, float * prev) {
    gpuUpdateBCsOnly<<<blocks_BC, threads_BC>>>(curr, prev, gx, gy, b, scaling_factor);
  }
};
